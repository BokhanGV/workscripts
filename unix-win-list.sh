...
textPathNotFound1="Could not find a part of the path"
textPathNotFound2="^Could not find file"

reAccessDeniedSimple="[A-Z]{1}[:][\\](.*){,}[\']"
reAccessDeniedVSS="[\\]{2}[?][\\]GLOBALROOT[\\]Device[\\]HarddiskVolumeShadowCopy[0-9]{,}[\\](.*){,}[\']"
reStorageCraft="[\\]{2}[?][\\]STC_SnapShot_Volume_[0-9]{,}_[0-9]{,}[\\](.*){,}[\']"
rePathNotFound="[\'](.*){,}[\']"

##### LOGIC

echo "Access denied:" > $workdir/$result
if [ "$2" = "dbg" ]; then xdg-open $workdir/$result; read -p "Created and opened. Press enter to continue"; fi #DBG
...
if [ "$2" = "dbg" ]; then xdg-open $workdir/$result; read -p "Simple done. Press enter to continue"; fi #DBG

grep "$textAccessDeniedVSS1"	$target	| grep -Eo "$reAccessDeniedVSS"		| sort | uniq | awk '1' RS='.\n' >> $workdir/$result	# for VSS paths
#grep "$textAccessDeniedVSS2"	$target	| grep -Eo "$reAccessDeniedVSS"		| sort | uniq | awk '1' RS='.\n' >> $workdir/$result	# for VSS paths - need to check whether this means "file not found" or "no access to it"
grep "$textAccessDeniedVSS2"	$target	| grep -Eo "$reStorageCraft"		| sort | uniq | awk '1' RS='.\n' >> $workdir/$result	# for VSS paths
if [ "$2" = "dbg" ]; then xdg-open $workdir/$result; read -p "VSS done. Press enter to continue"; fi #DBG
...
if [ "$2" = "dbg" ]; then xdg-open $workdir/$result; read -p "Not found done. Press enter to continue"; fi #DBG
